...
const KYC_STATUS_META: Record<string, { label: string; color: string }> = {
  ...
  needs_review: {
    label: "Needs review",
    color: "bg-gray-50 text-orange-800 border-gray-200",
  },
  expired: {
    label: "Expired",
...
const AML_STATUS_META: Record<string, { label: string; color: string }> = {
  ...
  reported: {
    label: "SAR Filed",
    color: "bg-gray-50 text-purple-800 border-gray-200",
  },
};
...
function OverviewTab({
  ...
  const kycData = [
    ...
    {
      name: "Needs review",
      value: kyc.filter((k) => k.status === "needs_review").length,
      fill: "#f97316",
    },
    {
      name: "Expired",
      value: kyc.filter((k) => k.status === "expired").length,
      fill: "#94a3b8",
    },
  ];
  ...
  const amlData = Object.entries(
    ...
    .sort(([, a], [, b]) => b - a)
    .map(([name, value]) => ({ name: AML_TYPE_LABEL[name] ?? name, value }));

  const riskData = [
    {
      name: "Low risk",
      value: kyc.filter((k) => k.riskTier === "low").length,
      fill: "#22c55e",
    },
    {
      name: "Medium risk",
      value: kyc.filter((k) => k.riskTier === "medium").length,
      fill: "#eab308",
    },
    {
      name: "High risk",
      value: kyc.filter((k) => k.riskTier === "high").length,
      fill: "#ef4444",
    },
  ];
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-4 gap-4">
        <MetricCard
          label="KYC approved"
          value={String(metrics.approvedKYC)}
          sub={`of ${metrics.totalKYCRecords} total`}
          accent="green"
        />
        <MetricCard
          label="Open AML alerts"
          value={String(metrics.openAMLAlerts)}
          sub="Requires investigation"
          accent={metrics.openAMLAlerts > 5 ? "red" : "yellow"}
        />
        <MetricCard
          label="SARs filed"
          value={String(metrics.sarsFiled)}
          sub="Suspicious Activity Reports"
          accent="purple"
        />
        <MetricCard
          label="Reserve held"
          value={formatCentsCompact(metrics.totalReserveHeld)}
          sub="Across all merchant reserves"
          accent="blue"
        />
      </div>
          ...
          {[
            {
              ...
              desc: "Verify identity of individuals. Passport, ID, address proof.",
            },
            {
              label: "KYB",
              full: "Know Your Business",
              color: "border-gray-200 bg-gray-50",
              desc: "Verify business entities, beneficial owners, and legitimacy.",
            },
            {
              label: "AML",
              full: "Anti-Money Laundering",
              color: "border-gray-200 bg-gray-50",
              desc: "Monitor transactions for suspicious patterns and report to authorities.",
            },
            {
              label: "Sanctions",
              full: "Sanctions screening",
              color: "border-red-200 bg-red-50",
              desc: "Screen against OFAC, EU, UN lists. Block prohibited entities.",
            },
          ].map((f) => (
            <div
              key={f.label}
              className={clsx("rounded-xl border p-4", f.color)}
            >
              ...
              {filtered.slice(0, 60).map((r) => (
                ...
                >
                  ...
                  <td className="px-4 py-2.5">
                    <span
                      className={clsx(
                        ...
                        RISK_TIER_META[r.riskTier]?.bg ?? "",
                        RISK_TIER_META[r.riskTier]?.color ?? "",
                      )}
                    >
                      {r.riskTier}
                    </span>
                  ...
                  <td className="px-4 py-2.5 text-slate-500">
                    {r.documentsSubmitted.length} docs
                  </td>
          ...
          <div className="rounded-xl border border-slate-200 bg-white shadow-sm p-5 sticky top-4 space-y-4">
            <div>
              ...
              <p className="text-xs text-slate-400 mt-0.5">
                {selected.id} · {selected.customerId}
              </p>
              <div className="flex items-center gap-2 mt-2">
                ...
                <span
                  className={clsx(
                    ...
                    RISK_TIER_META[selected.riskTier]?.bg,
                    RISK_TIER_META[selected.riskTier]?.color,
                  )}
                >
...
function AMLTab({ alerts }: { alerts: AMLAlert[] }) {
  ...
  const open = alerts.filter((a) =>
    ["open", "investigating", "escalated"].includes(a.status),
  );
        ...
        {open.length > 0 && (
          <div className="rounded-xl border border-red-200 bg-red-50 p-4 flex items-center gap-3">
            ...
            <p className="text-sm font-medium text-red-800">
              {open.length} active AML alerts require investigation —{" "}
              {alerts.filter((a) => a.status === "escalated").length} escalated
            </p>
          </div>
        )}
              ...
              {filtered.slice(0, 60).map((a) => (
                ...
                >
                  ...
                  <td className="px-3 py-2.5 font-semibold text-slate-900">
                    {formatCentsCompact(a.totalAmount)}
                  </td>
                  <td className="px-3 py-2.5 text-slate-600">
                    {a.transactionCount}
                  </td>
                  <td className="px-3 py-2.5">
                    {a.sarFiled && (
                      <span className="rounded bg-gray-50 px-1.5 py-0.5 text-[10px] font-semibold text-purple-700">
                        SAR
                      </span>
                    )}
                  </td>
                  <td className="px-3 py-2.5">
                    <AMLBadge status={a.status} />
                  </td>
          ...
          <div className="rounded-xl border border-slate-200 bg-white shadow-sm p-5 sticky top-4 space-y-4 max-h-[85vh] overflow-y-auto">
            <div>
              ...
              <div className="flex items-center gap-2 mt-2 flex-wrap">
                ...
                {selected.sarFiled && (
                  <span className="rounded-full bg-gray-50 border border-gray-200 px-2 py-0.5 text-xs font-semibold text-purple-700">
                    SAR Filed
                  </span>
                )}
              </div>
            ...
            <div className="rounded-lg bg-gray-50 border border-gray-200 p-3">
              <p className="text-xs font-semibold text-amber-800 mb-1">
                {AML_TYPE_LABEL[selected.alertType]}
              </p>
            ...
            <div className="rounded-lg bg-slate-50 border border-slate-200 p-3">
              ...
              <p className="text-xs text-slate-500 leading-relaxed">
                If suspicious activity is confirmed, a Suspicious Activity
                Report (SAR) must be filed with FinCEN within 30 days of
                detection. Filing is confidential — the customer must not be
                tipped off ("tipping off" is a criminal offence).
              </p>
            </div>
          </div>
        </div>
    ...
    <div className="space-y-4">
      <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
        <p className="text-xs text-slate-500 leading-relaxed max-w-3xl">
          Merchant reserves protect the payment processor against chargeback
          losses. When a merchant has elevated risk (new business, high
          chargeback rate, or high-risk MCC), the processor withholds a
          percentage of settlement funds — typically 5–10% — held in a reserve
          account. Reserves are released on a schedule once risk subsides.
        </p>
      </div>
        ...
        {[
          {
            ...
            desc: "Fixed % of each settlement withheld, released after 90–180 days",
            color: "border-l-gray-300",
          },
          {
            label: "Fixed reserve",
            desc: "Fixed lump sum withheld upfront, released when contract ends",
            color: "border-l-gray-300",
          },
          {
            label: "Capped reserve",
            desc: "Rolling reserve up to a maximum cap, then no further withholding",
            color: "border-l-gray-300",
          },
        ].map((r) => (
          <div
            key={r.label}
            className={`rounded-xl border border-slate-200 bg-white p-4 shadow-sm border-l-4 ${r.color}`}
          >
              ...
              return (
                ...
                >
                  ...
                  <td className="px-4 py-2.5 font-semibold text-slate-900">
                    {formatPercent(r.reserveRate, 1)}
                  </td>
                  <td className="px-4 py-2.5 font-semibold text-slate-900">
                    {formatCentsCompact(r.currentBalance)}
                  </td>
                  <td className="px-4 py-2.5 text-slate-600">
                    {formatCentsCompact(r.requiredBalance)}
                  </td>
                  <td className="px-4 py-2.5">
                  ...
                  <td className="px-4 py-2.5 text-slate-400">
                    {formatDate(r.nextReleaseDate)}
                  </td>
                  <td className="px-4 py-2.5 text-green-600 font-medium">
                    {formatCentsCompact(r.nextReleaseAmount)}
                  </td>
                </tr>
              );
            })}
...
function TreasuryTab({ positions }: { positions: TreasuryPosition[] }) {
  ...
  return (
    <div className="space-y-5">
      <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
        <p className="text-xs text-slate-500 leading-relaxed max-w-3xl">
          Treasury operations manage the actual cash positions across currencies
          and correspondent accounts. Nostro accounts hold funds at
          correspondent banks in local currencies. Prefunding ensures funds are
          available before outgoing payments settle. Intraday liquidity must be
          managed carefully — a shortfall can delay all outgoing payments.
        </p>
      </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="rounded-xl border border-l-4 border-gray-200 bg-white p-4 shadow-sm">
          <p className="text-xs font-medium uppercase tracking-wide text-slate-400">
            Total nostro balance
          </p>
          <p className="mt-1 text-xl font-semibold text-slate-900">
            {formatCentsCompact(totalNostro)}
          </p>
          <p className="text-xs text-slate-400 mt-0.5">
            Across {positions.length} currencies
          </p>
        </div>
        <div className="rounded-xl border border-l-4 border-green-500 bg-white p-4 shadow-sm">
          ...
          <p className="mt-1 text-xl font-semibold text-slate-900">
            {formatCentsCompact(
              positions.reduce((s, p) => s + p.availableAmount, 0),
            )}
          </p>
          <p className="text-xs text-slate-400 mt-0.5">
            After prefunding and reserves
          </p>
        </div>
        <div className="rounded-xl border border-l-4 border-gray-200 bg-white p-4 shadow-sm">
          <p className="text-xs font-medium uppercase tracking-wide text-slate-400">
          ...
          <p className="mt-1 text-xl font-semibold text-slate-900">
            {formatCentsCompact(
              positions.reduce((s, p) => s + p.pendingOutbound, 0),
            )}
          </p>
          ...
          <tbody>
            {positions.map((p) => (
              ...
              >
                ...
                <td className="px-4 py-2.5 font-semibold text-slate-900">
                  {formatCentsCompact(p.nostroBalance)}
                </td>
                <td className="px-4 py-2.5 text-blue-600">
                  {formatCentsCompact(p.prefundedAmount)}
                </td>
                <td className="px-4 py-2.5 text-purple-600">
                  {formatCentsCompact(p.reservedAmount)}
                </td>
                <td
                  className={clsx(
                    "px-4 py-2.5 font-semibold",
                    p.availableAmount < p.pendingOutbound
                      ? "text-red-600"
                      : "text-green-600",
                  )}
                >
                ...
                <td className="px-4 py-2.5 text-green-600">
                  +{formatCentsCompact(p.pendingInbound)}
                </td>
                <td className="px-4 py-2.5 text-red-600">
                  −{formatCentsCompact(p.pendingOutbound)}
                </td>
                <td className="px-4 py-2.5 text-slate-400">
...
export default function CompliancePage() {
  ...
  return (
    <PageShell
      ...
      subtitle="KYC/KYB · AML monitoring · SARs · Reserves · Prefunding · Treasury"
    ...
    >
      {tab === "Overview" && (
        <OverviewTab kyc={kyc} aml={aml} reserves={reserves} />
      )}
