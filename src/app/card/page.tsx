...
function buildVolumeChart(txns: CardTransaction[]) {
  ...
  txns.forEach((t) => {
    ...
    buckets[key] =
      (buckets[key] ?? 0) + (t.status !== "declined" ? t.amount : 0);
  });
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-4 gap-4">
        ...
        <MetricCard
          ...
          trendLabel={
            metrics.approvalRate > 0.88 ? "Above target" : "Below 88% target"
          }
        />
        <MetricCard
              ...
              <PieChart>
                ...
                >
                  {networkData.map((entry) => (
                    <Cell
                      key={entry.name}
                      fill={NETWORK_COLORS[entry.name] ?? "#94a3b8"}
                    />
                  ))}
                </Pie>
              </PieChart>
              ...
              {networkData.map((d) => (
                <div key={d.name} className="flex items-center gap-2 text-xs">
                  <span
                    ...
                    style={{ background: NETWORK_COLORS[d.name] }}
                  />
                  <span className="text-slate-600 w-20">
                    {NETWORK_LABEL[d.name]}
                  </span>
            ...
            <div className="space-y-2">
              {statusData.map((d) => (
                <div key={d.name} className="flex items-center gap-2 text-xs">
                  ...
                  />
                  <span className="text-slate-600 w-20 capitalize">
                    {d.name}
                  </span>
                  <span className="font-semibold text-slate-900">
                    {d.value}
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          {[
            {
              ...
              value: formatCentsCompact(metrics.totalFees * 0.78),
              color: "text-blue-600",
            },
            {
              label: "Scheme fees",
              value: formatCentsCompact(metrics.totalFees * 0.08),
              color: "text-purple-600",
            },
            {
              label: "Processing fees",
              value: formatCentsCompact(metrics.totalFees * 0.14),
              color: "text-slate-600",
            },
          ].map((f) => (
            <div key={f.label} className="rounded-lg bg-slate-50 p-4">
              <p className="text-xs text-slate-500">{f.label}</p>
              <p className={`mt-1 text-lg font-semibold ${f.color}`}>
                {f.value}
              </p>
            </div>
          ))}
        ...
        <div className="mt-4 rounded-lg bg-slate-900 p-4 flex items-center justify-between">
          ...
          <span className="text-lg font-semibold text-white">
            {formatCentsCompact(metrics.netRevenue)}
          </span>
        </div>
      </div>
    </div>
        ...
        <div className="flex gap-2 mb-4 flex-wrap">
          {[
            ...
            >
              {s === "all"
                ? "All"
                : CARD_STATUS_LABEL[s as keyof typeof CARD_STATUS_LABEL]}
            </button>
          ))}
        </div>
          ...
          <div className="rounded-xl border border-slate-200 bg-white shadow-sm p-5 sticky top-4 space-y-5">
            <div className="flex items-start justify-between">
              ...
              <StatusBadge
                ...
                type="card"
                label={CARD_STATUS_LABEL[selected.status]}
              />
            </div>
            ...
            {selected.declineReason && (
              <div className="rounded-lg bg-red-50 border border-red-100 p-3">
                ...
                <p className="text-xs text-red-600 mt-0.5">
                  {DECLINE_REASON_LABEL[selected.declineReason]}
                </p>
              </div>
            )}
            ...
            <div className="border-t border-slate-100 pt-4 space-y-2">
              ...
              <div className="flex justify-between text-xs border-t border-slate-100 pt-2">
                ...
                <span className="font-semibold text-green-600">
                  {formatCents(selected.netSettlement)}
                </span>
              </div>
            </div>
              ...
              {[
                ...
                <div key={label} className="flex justify-between text-xs">
                  <span
                    className={clsx(
                      "",
                      ts ? "text-slate-400" : "text-slate-200",
                    )}
                  >
                    {label}
                  </span>
    ...
    <div className="space-y-4">
      <div className="grid grid-cols-3 gap-4">
        {[
          {
            label: "Open cases",
            value: chargebacks.filter(
              (c) => !["won", "lost"].includes(c.status),
            ).length,
            color: "border-l-yellow-500",
          },
          {
            label: "Won",
            value: chargebacks.filter((c) => c.status === "won").length,
            color: "border-l-green-500",
          },
          {
            label: "Lost",
            value: chargebacks.filter((c) => c.status === "lost").length,
              ...
              >
                ...
                <td className="px-4 py-3">
                  <StatusBadge
                    status={cb.status}
                    type="chargeback"
                    label={CHARGEBACK_STATUS_LABEL[cb.status]}
                    size="sm"
                  />
                </td>
                <td className="px-4 py-3 text-slate-400">
                  {formatDate(cb.deadlineAt)}
                </td>
              </tr>
            ))}
          </tbody>
          ...
          <tbody>
            {batches.map((b) => (
              ...
              >
                ...
                <td className="px-4 py-3 font-semibold text-slate-900">
                  {formatCentsCompact(b.grossAmount)}
                </td>
                <td className="px-4 py-3 text-red-600">
                  −{formatCentsCompact(b.totalFees)}
                </td>
                <td className="px-4 py-3 font-semibold text-green-600">
                  {formatCentsCompact(b.netAmount)}
                </td>
                ...
                <td className="px-4 py-3">
                  <StatusBadge
                    status={b.status}
                    type="settlement"
                    label={SETTLEMENT_STATUS_LABEL[b.status]}
                    size="sm"
                  />
                </td>
              </tr>
