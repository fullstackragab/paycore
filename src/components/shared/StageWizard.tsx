        ...
        <div style={{ display: "flex", alignItems: "center", gap: 0, overflowX: "auto", paddingBottom: 4 }}>
            ...
            <div key={stage.id} style={{ display: "flex", alignItems: "center", flexShrink: 0 }}>
              <button
                ...
                style={{
                  ...
                  padding: "6px 12px", borderRadius: 4, border: "none", background: "none",
                  cursor: "pointer", minWidth: 80,
                  background: displayed?.id === stage.id ? "#f9fafb" : "none",
              ...
              {i < stages.length - 1 && (
                <div style={{
                  ...
                  background: stages[i + 1]?.status === "pending" ? "#e5e7eb" : "#374151",
                }} />
              )}
            </div>
          ))}
          {processing && (
            <div style={{ display: "flex", alignItems: "center", flexShrink: 0, marginLeft: 8 }}>
              <span style={{ fontSize: 12, color: "#9ca3af" }}>Processing...</span>
            </div>
          )}
        </div>
      </div>
      ...
      {displayed && (
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
          ...
          <div style={{ background: "#fff", border: "1px solid #e5e7eb", borderRadius: 6, overflow: "hidden" }}>
            ...
            <div style={{ padding: 24, minHeight: 200, display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center", gap: 12 }}>
              <div style={{
                width: 48, height: 48, borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 22,
                background: displayed.status === "success" ? "#f0fdf4" : displayed.status === "failed" ? "#fef2f2" : displayed.status === "warning" ? "#fffbeb" : "#f9fafb",
            ...
            <div style={{ padding: 16, display: "flex", flexDirection: "column", gap: 12 }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                {displayed.system.fields.map((f) => (
                  <div key={f.label} style={{ display: "flex", gap: 8, fontSize: 11 }}>
                    <span style={{ color: "#9ca3af", flexShrink: 0, width: 140, fontFamily: "monospace" }}>{f.label}</span>
                    <span style={{ color: "#374151", fontFamily: "monospace", wordBreak: "break-all", whiteSpace: "pre-line" }}>
                      {f.sensitive ? `${String(f.value).slice(0,4)}...` : f.value}
                    </span>
                  </div>
                ))}
              </div>
              <div style={{ borderTop: "1px solid #f3f4f6", paddingTop: 10, fontSize: 11, color: "#6b7280", lineHeight: 1.6, fontStyle: "italic" }}>
